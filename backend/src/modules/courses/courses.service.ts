...
export class CoursesService {
    ...
    if (user.role === "TEACHER") {
      ...
      if (teacherId == null) {
        throw new ForbiddenException(
          "teacherId is required when creating a course as admin",
        );
      }
    } else {
      throw new ForbiddenException(
        "Insufficient permissions to create a course",
      );
    }
  ...
  async update(id: number, dto: UpdateCourseDto, user: JwtUser) {
    ...
    if (user.role === "TEACHER") {
      ...
      throw new ForbiddenException(
        "Insufficient permissions to update a course",
      );
    }
  ...
  async remove(id: number, user: JwtUser) {
    ...
    if (user.role !== "ADMIN" && user.role !== "TEACHER") {
      throw new ForbiddenException(
        "Insufficient permissions to delete a course",
      );
    }
